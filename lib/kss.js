...
parseChunk = function(data, input, options) {
	var currSection, i, l, blocks = [], content = [], paragraphs, j, m, allModifiers;
	...
	content = findBlocks(input, options);
	blocks = content.comments;
	l = blocks.length;
	...
	for (i = 0; i < l; i += 1) {
		...
		currSection.source = content.source;
...
findBlocks = function(input, options) {
	...
	for (i = 0; i < l; i += 1) {
		...
		//add non comment blocks as source
		source += line.replace(/\s*?\/\/([ ]{0,1}|\t)/, '') + '\n';
	}
	...
	return {comments: blocks, source: source};
};
